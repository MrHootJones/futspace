...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    struct futhark_i32_2d *read_value_4817;
    int64_t read_shape_4818[2];
    int32_t *read_arr_4819 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4819, read_shape_4818, 2) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0, "[][]",
              i32_info.type_name, strerror(errno));
    
    struct futhark_i32_2d *read_value_4820;
    int64_t read_shape_4821[2];
    int32_t *read_arr_4822 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4822, read_shape_4821, 2) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1, "[][]",
              i32_info.type_name, strerror(errno));
    
    struct futhark_i32_2d *result_4823;
    
    if (perform_warmup) {
        int r;
        
        assert((read_value_4817 = futhark_new_i32_2d(ctx, read_arr_4819,
                                                     read_shape_4818[0],
                                                     read_shape_4818[1])) != 0);
        assert((read_value_4820 = futhark_new_i32_2d(ctx, read_arr_4822,
                                                     read_shape_4821[0],
                                                     read_shape_4821[1])) != 0);
        ...
        r = futhark_entry_main(ctx, &result_4823, read_value_4817,
                               read_value_4820);
        if (r != 0)
            panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
        ...
        assert(futhark_free_i32_2d(ctx, read_value_4817) == 0);
        assert(futhark_free_i32_2d(ctx, read_value_4820) == 0);
        assert(futhark_free_i32_2d(ctx, result_4823) == 0);
    }
    time_runs = 1;
    /* Proper run. */
        ...
        set_binary_mode(stdout);
    {
        int32_t *arr = calloc(sizeof(int32_t), futhark_shape_i32_2d(ctx,
                                                                    result_4823)[0] *
                              futhark_shape_i32_2d(ctx, result_4823)[1]);
        
        assert(arr != NULL);
        assert(futhark_values_i32_2d(ctx, result_4823, arr) == 0);
        write_array(stdout, binary_output, &i32_info, arr,
                    futhark_shape_i32_2d(ctx, result_4823), 2);
        free(arr);
    }
    printf("\n");
    assert(futhark_free_i32_2d(ctx, result_4823) == 0);
...
struct futhark_context_config *futhark_context_config_new(void)
{
    ...
    opencl_config_init(&cfg->opencl, 0, size_names, size_vars, cfg->sizes,
                       size_classes);
    return cfg;
}
void futhark_context_config_free(struct futhark_context_config *cfg)
...
static int futrts_main(struct futhark_context *ctx,
                       struct memblock_device *out_mem_p_4814,
                       int32_t *out_out_arrsizze_4815,
                       int32_t *out_out_arrsizze_4816,
                       struct memblock_device color_map_mem_4803,
                       struct memblock_device height_map_mem_4804,
                       int32_t h_4792, int32_t w_4793, int32_t h_4794,
                       int32_t w_4795);
static inline uint8_t add8(uint8_t x, uint8_t y)
{
    return x + y;
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       int32_t w_4795)
{
    struct memblock_device out_mem_4811;
    
    out_mem_4811.references = NULL;
    
    int32_t out_arrsizze_4812;
    int32_t out_arrsizze_4813;
    bool dim_match_4798 = h_4792 == h_4794;
    bool dim_match_4799 = w_4793 == w_4795;
    bool match_4800 = dim_match_4798 && dim_match_4799;
    bool empty_or_match_cert_4801;
    
    if (!match_4800) {
        ctx->error = msgprintf("Error: %s\n\nBacktrace:\n%s",
                               "function arguments of wrong shape",
                               "-> #0  height_calculation.fut:60:1-92:17\n");
        if (memblock_unref_device(ctx, &out_mem_4811, "out_mem_4811") != 0)
            return 1;
        return 1;
    }
    
    int64_t binop_x_4806 = sext_i32_i64(h_4792);
    int64_t binop_y_4807 = sext_i32_i64(w_4793);
    int64_t binop_x_4808 = mul64(binop_x_4806, binop_y_4807);
    int64_t bytes_4805 = mul64(4, binop_x_4808);
    struct memblock_device mem_4809;
    
    mem_4809.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4809, bytes_4805, "mem_4809"))
        return 1;
    if (mul64(sext_i32_i64(mul32(h_4792, w_4793)), (int32_t) sizeof(int32_t)) >
        0) {
        OPENCL_SUCCEED_OR_RETURN(clEnqueueCopyBuffer(ctx->opencl.queue,
                                                     height_map_mem_4804.mem,
                                                     mem_4809.mem, 0, 0,
                                                     mul64(sext_i32_i64(mul32(h_4792,
                                                                              w_4793)),
    ...
    out_arrsizze_4812 = h_4792;
    out_arrsizze_4813 = w_4793;
    if (memblock_set_device(ctx, &out_mem_4811, &mem_4809, "mem_4809") != 0)
        return 1;
    (*out_mem_p_4814).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4814, &out_mem_4811,
                            "out_mem_4811") != 0)
        return 1;
    *out_out_arrsizze_4815 = out_arrsizze_4812;
    *out_out_arrsizze_4816 = out_arrsizze_4813;
    if (memblock_unref_device(ctx, &mem_4809, "mem_4809") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4811, "out_mem_4811") != 0)
        return 1;
    return 0;
                       ...
                       struct futhark_i32_2d *in1)
{
    ...
    color_map_mem_4803.references = NULL;
    
    struct memblock_device height_map_mem_4804;
    
    height_map_mem_4804.references = NULL;
    ...
    lock_lock(&ctx->lock);
    color_map_mem_4803 = in0->mem;
    h_4792 = in0->shape[0];
    w_4793 = in0->shape[1];
    height_map_mem_4804 = in1->mem;
    h_4794 = in1->shape[0];
    w_4795 = in1->shape[1];
    
    int ret = futrts_main(ctx, &out_mem_4811, &out_arrsizze_4812,
                          &out_arrsizze_4813, color_map_mem_4803,
                          height_map_mem_4804, h_4792, w_4793, h_4794, w_4795);
    
    if (ret == 0) {
        assert((*out0 =
                (struct futhark_i32_2d *) malloc(sizeof(struct futhark_i32_2d))) !=
            NULL);
        (*out0)->mem = out_mem_4811;
        (*out0)->shape[0] = out_arrsizze_4812;
        (*out0)->shape[1] = out_arrsizze_4813;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
