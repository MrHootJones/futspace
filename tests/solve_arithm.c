...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    float read_value_4367;
    
    if (read_scalar(&f32_info, &read_value_4367) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 0,
              f32_info.type_name, strerror(errno));
    
    float read_value_4368;
    
    if (read_scalar(&f32_info, &read_value_4368) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 1,
              f32_info.type_name, strerror(errno));
    
    float read_value_4369;
    
    if (read_scalar(&f32_info, &read_value_4369) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 2,
              f32_info.type_name, strerror(errno));
    
    struct futhark_f32_1d *result_4370;
    ...
    if (perform_warmup) {
        ...
        r = futhark_entry_main(ctx, &result_4370, read_value_4367,
                               read_value_4368, read_value_4369);
        if (r != 0)
            panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
            panic(1, "%s", futhark_context_get_error(ctx));
        ...
        ;
        ;
        assert(futhark_free_f32_1d(ctx, result_4370) == 0);
    }
    time_runs = 1;
    /* Proper run. */
        ...
        set_binary_mode(stdout);
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_f32_1d(ctx, result_4370, arr) == 0);
        write_array(stdout, binary_output, &f32_info, arr,
                    futhark_shape_f32_1d(ctx, result_4370), 1);
        free(arr);
    }
    printf("\n");
    assert(futhark_free_f32_1d(ctx, result_4370) == 0);
...
static const char *opencl_program[] =
                  {"#ifdef cl_clang_storage_class_specifiers\n#pragma OPENCL EXTENSION cl_clang_storage_class_specifiers : enable\n#endif\n#pragma OPENCL EXTENSION cl_khr_byte_addressable_store : enable\n__kernel void dummy_kernel(__global unsigned char *dummy, int n)\n{\n    const int thread_gid = get_global_id(0);\n    \n    if (thread_gid >= n)\n        return;\n}\ntypedef char int8_t;\ntypedef short int16_t;\ntypedef int int32_t;\ntypedef long int64_t;\ntypedef uchar uint8_t;\ntypedef ushort uint16_t;\ntypedef uint uint32_t;\ntypedef ulong uint64_t;\n#ifdef cl_nv_pragma_unroll\nstatic inline void mem_fence_global()\n{\n    asm(\"membar.gl;\");\n}\n#else\nstatic inline void mem_fence_global()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);\n}\n#endif\nstatic inline void mem_fence_local()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE);\n}\nstatic inline uint8_t add8(uint8_t x, uint8_t y)\n{\n    return x + y;\n}\nstatic inline uint16_t add16(uint16_t x, uint16_t y)\n{\n    return x + y;\n}\nstatic inline uint32_t add32(uint32_t x, uint32_t y)\n{\n    return x + y;\n}\nstatic inline uint64_t add64(uint64_t x, uint64_t y)\n{\n    return x + y;\n}\nstatic inline uint8_t sub8(uint8_t x, uint8_t y)\n{\n    return x - y;\n}\nstatic inline uint16_t sub16(uint16_t x, uint16_t y)\n{\n    return x - y;\n}\nstatic inline uint32_t sub32(uint32_t x, uint32_t y)\n{\n    return x - y;\n}\nstatic inline uint64_t sub64(uint64_t x, uint64_t y)\n{\n    return x - y;\n}\nstatic inline uint8_t mul8(uint8_t x, uint8_t y)\n{\n    return x * y;\n}\nstatic inline uint16_t mul16(uint16_t x, uint16_t y)\n{\n    return x * y;\n}\nstatic inline uint32_t mul32(uint32_t x, uint32_t y)\n{\n    return x * y;\n}\nstatic inline uint64_t mul64(uint64_t x, uint64_t y)\n{\n    return x * y;\n}\nstatic inline uint8_t udiv8(uint8_t x, uint8_t y)\n{\n    return x / y;\n}\nstatic inline uint16_t udiv16(uint16_t x, uint16_t y)\n{\n    return x / y;\n}\nstatic inline uint32_t udiv32(uint32_t x, uint32_t y)\n{\n    return x / y;\n}\nstatic inline uint64_t udiv64(uint64_t x, uint64_t y)\n{\n    return x / y;\n}\nstatic ",
                   ...
                   "rts_sin32(float x)\n{\n    return sin(x);\n}\nstatic inline float futrts_tan32(float x)\n{\n    return tan(x);\n}\nstatic inline float futrts_acos32(float x)\n{\n    return acos(x);\n}\nstatic inline float futrts_asin32(float x)\n{\n    return asin(x);\n}\nstatic inline float futrts_atan32(float x)\n{\n    return atan(x);\n}\nstatic inline float futrts_atan2_32(float x, float y)\n{\n    return atan2(x, y);\n}\nstatic inline float futrts_gamma32(float x)\n{\n    return tgamma(x);\n}\nstatic inline float futrts_lgamma32(float x)\n{\n    return lgamma(x);\n}\nstatic inline bool futrts_isnan32(float x)\n{\n    return isnan(x);\n}\nstatic inline bool futrts_isinf32(float x)\n{\n    return isinf(x);\n}\nstatic inline int32_t futrts_to_bits32(float x)\n{\n    union {\n        float f;\n        int32_t t;\n    } p;\n    \n    p.f = x;\n    return p.t;\n}\nstatic inline float futrts_from_bits32(int32_t x)\n{\n    union {\n        int32_t f;\n        float t;\n    } p;\n    \n    p.f = x;\n    return p.t;\n}\n#ifdef __OPENCL_VERSION__\nstatic inline float fmod32(float x, float y)\n{\n    return fmod(x, y);\n}\nstatic inline float futrts_round32(float x)\n{\n    return rint(x);\n}\nstatic inline float futrts_floor32(float x)\n{\n    return floor(x);\n}\nstatic inline float futrts_ceil32(float x)\n{\n    return ceil(x);\n}\nstatic inline float futrts_lerp32(float v0, float v1, float t)\n{\n    return mix(v0, v1, t);\n}\n#else\nstatic inline float fmod32(float x, float y)\n{\n    return fmodf(x, y);\n}\nstatic inline float futrts_round32(float x)\n{\n    return rintf(x);\n}\nstatic inline float futrts_floor32(float x)\n{\n    return floorf(x);\n}\nstatic inline float futrts_ceil32(float x)\n{\n    return ceilf(x);\n}\nstatic inline float futrts_lerp32(float v0, float v1, float t)\n{\n    return v0 + (v1 - v0) * t;\n}\n#endif\n__kernel void segmap_4327(__global int *global_failure, float d_4297,\n                          float y_4298, int32_t res_4310, __global\n                          unsigned char *mem_4350)\n{\n    #define segmap_group_sizze_4331 (mainzisegmap_group_sizze_4330)\n",
                   "    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4355;\n    int32_t local_tid_4356;\n    int32_t group_sizze_4359;\n    int32_t wave_sizze_4358;\n    int32_t group_tid_4357;\n    \n    global_tid_4355 = get_global_id(0);\n    local_tid_4356 = get_local_id(0);\n    group_sizze_4359 = get_local_size(0);\n    wave_sizze_4358 = LOCKSTEP_WIDTH;\n    group_tid_4357 = get_group_id(0);\n    \n    int32_t phys_tid_4327 = global_tid_4355;\n    int32_t gtid_4326 = add32(mul32(group_tid_4357, segmap_group_sizze_4331),\n                              local_tid_4356);\n    \n    if (slt32(gtid_4326, res_4310)) {\n        int32_t index_primexp_4346 = add32(1, gtid_4326);\n        float res_4339 = sitofp_i32_f32(index_primexp_4346);\n        float x_4340 = res_4339 / 2.0F;\n        float x_4341 = res_4339 - 1.0F;\n        float y_4342 = d_4297 * x_4341;\n        float y_4343 = y_4298 + y_4342;\n        float res_4344 = x_4340 * y_4343;\n        \n        ((__global float *) mem_4350)[gtid_4326] = res_4344;\n    }\n    \n  error_0:\n    return;\n    #undef segmap_group_sizze_4331\n}\n",
                   NULL};
...
static const char *size_names[] = {"main.segmap_group_size_4330"};
static const char *size_vars[] = {"mainzisegmap_group_sizze_4330"};
...
struct sizes {
    size_t mainzisegmap_group_sizze_4330;
} ;
struct futhark_context_config {
...
struct futhark_context {
    ...
    int64_t segmap_4327_total_runtime;
    int segmap_4327_runs;
                               ...
                               struct futhark_context *ctx)
{
    ...
    ctx->segmap_4327_total_runtime = 0;
    ctx->segmap_4327_runs = 0;
}
                             ...
                             struct futhark_context *ctx, cl_program prog)
{
    ...
    OPENCL_SUCCEED_OR_RETURN(error);
    {
        ctx->segmap_4327 = clCreateKernel(prog, "segmap_4327", &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->segmap_4327, 0, sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
            fprintf(stderr, "Created kernel %s.\n", "segmap_4327");
    }
    ctx->sizes.mainzisegmap_group_sizze_4330 = cfg->sizes[0];
    return 0;
}
struct futhark_context *futhark_context_new(struct futhark_context_config *cfg)
{
...
void futhark_debugging_report(struct futhark_context *ctx)
{
    ...
    if (ctx->profiling) {
        ...
        fprintf(stderr,
                "segmap_4327          ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->segmap_4327_runs, (long) ctx->segmap_4327_total_runtime /
                (ctx->segmap_4327_runs != 0 ? ctx->segmap_4327_runs : 1),
                (long) ctx->segmap_4327_total_runtime);
        ctx->total_runtime += ctx->segmap_4327_total_runtime;
        ctx->total_runs += ctx->segmap_4327_runs;
        if (ctx->profiling)
...
static int futrts_main(struct futhark_context *ctx,
                       struct memblock_device *out_mem_p_4360,
                       int32_t *out_out_arrsizze_4361, float c_4295,
                       float zz_0_4296, float d_4297);
static inline uint8_t add8(uint8_t x, uint8_t y)
{
    return x + y;
}
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       float zz_0_4296, float d_4297)
{
    struct memblock_device out_mem_4353;
    
    out_mem_4353.references = NULL;
    
    int32_t out_arrsizze_4354;
    float y_4298 = 2.0F * zz_0_4296;
    float x_4299 = d_4297 - y_4298;
    float x_4300 = fpow32(x_4299, 2.0F);
    float x_4301 = 8.0F * d_4297;
    float y_4302 = c_4295 * x_4301;
    float sqrt_arg_4303 = x_4300 + y_4302;
    float res_4304;
    
    res_4304 = futrts_sqrt32(sqrt_arg_4303);
    
    float x_4305 = res_4304 - y_4298;
    float num_4306 = d_4297 + x_4305;
    float div_4307 = 2.0F * d_4297;
    float lifted_0_floor_arg_4308 = num_4306 / div_4307;
    float res_4309;
    
    res_4309 = futrts_floor32(lifted_0_floor_arg_4308);
    
    int32_t res_4310 = fptosi_f32_i32(res_4309);
    bool bounds_invalid_upwards_4311 = slt32(res_4310, 1);
    bool valid_4314 = !bounds_invalid_upwards_4311;
    bool range_valid_c_4315;
    
    if (!valid_4314) {
        ctx->error = msgprintf("Error: %s%d%s%d%s\n\nBacktrace:\n%s", "Range ",
                               1, "...", res_4310, " is invalid.",
                               "-> #0  solve_arithm.fut:7:37-41\n   #1  solve_arithm.fut:1:1-8:49\n");
        if (memblock_unref_device(ctx, &out_mem_4353, "out_mem_4353") != 0)
            return 1;
        return 1;
    }
    
    int64_t res_4328 = sext_i32_i64(res_4310);
    int32_t segmap_group_sizze_4331;
    
    segmap_group_sizze_4331 = ctx->sizes.mainzisegmap_group_sizze_4330;
    
    int64_t segmap_group_sizze_4332 = sext_i32_i64(segmap_group_sizze_4331);
    int64_t y_4333 = sub64(segmap_group_sizze_4332, 1);
    int64_t x_4334 = add64(res_4328, y_4333);
    int64_t segmap_usable_groups_64_4336 = squot64(x_4334,
                                                   segmap_group_sizze_4332);
    int32_t segmap_usable_groups_4337 =
            sext_i64_i32(segmap_usable_groups_64_4336);
    int64_t bytes_4348 = mul64(4, res_4328);
    struct memblock_device mem_4350;
    
    mem_4350.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4350, bytes_4348, "mem_4350"))
        return 1;
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4327, 1, sizeof(d_4297),
                                            &d_4297));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4327, 2, sizeof(y_4298),
                                            &y_4298));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4327, 3,
                                            sizeof(res_4310), &res_4310));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4327, 4,
                                            sizeof(mem_4350.mem),
                                            &mem_4350.mem));
    if (1 * (segmap_usable_groups_4337 * segmap_group_sizze_4331) != 0) {
        const size_t global_work_sizze_4362[1] = {segmap_usable_groups_4337 *
                     segmap_group_sizze_4331};
        const size_t local_work_sizze_4366[1] = {segmap_group_sizze_4331};
        int64_t time_start_4363 = 0, time_end_4364 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "segmap_4327");
            fprintf(stderr, "%zu", global_work_sizze_4362[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4366[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) 0);
            time_start_4363 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->segmap_4327, 1,
                                                        NULL,
                                                        global_work_sizze_4362,
                                                        local_work_sizze_4366,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->segmap_4327_runs,
                                                                                                  &ctx->segmap_4327_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4364 = get_wall_time();
            
            long time_diff_4365 = time_end_4364 - time_start_4363;
            
            fprintf(stderr, "kernel %s runtime: %ldus\n", "segmap_4327",
                    time_diff_4365);
        }
    }
    out_arrsizze_4354 = res_4310;
    if (memblock_set_device(ctx, &out_mem_4353, &mem_4350, "mem_4350") != 0)
        return 1;
    (*out_mem_p_4360).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4360, &out_mem_4353,
                            "out_mem_4353") != 0)
        return 1;
    *out_out_arrsizze_4361 = out_arrsizze_4354;
    if (memblock_unref_device(ctx, &mem_4350, "mem_4350") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4353, "out_mem_4353") != 0)
        return 1;
    return 0;
                       ...
                       float in1, const float in2)
{
    ...
    int ret = futrts_main(ctx, &out_mem_4353, &out_arrsizze_4354, c_4295,
                          zz_0_4296, d_4297);
    ...
    if (ret == 0) {
        assert((*out0 =
                (struct futhark_f32_1d *) malloc(sizeof(struct futhark_f32_1d))) !=
            NULL);
        (*out0)->mem = out_mem_4353;
        (*out0)->shape[0] = out_arrsizze_4354;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
